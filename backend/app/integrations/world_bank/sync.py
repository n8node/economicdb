...
from app.integrations.world_bank.client import WorldBankError, fetch_indicator_series
from app.integrations.world_bank.mappings import WORLD_BANK_MAPPINGS
...
async def sync_world_bank(session: AsyncSession, provider: DataProvider) -> dict:
    total_records = 0
    synced_indicators: list[str] = []

    try:
        for mapping in WORLD_BANK_MAPPINGS:
            indicator = await session.get(Indicator, mapping.indicator_id)
            if indicator is None:
                logger.warning(
                    "world_bank_sync_skip_unknown_indicator",
                    indicator_id=mapping.indicator_id,
                )
                continue

            if mapping.series_type == "gdp_growth":
                series = await fetch_indicator_series(mapping.country_code, mapping.indicator_code)
            else:
                continue
        ...
        return {
            ...
            "message": f"World Bank: загружено {total_records} точек для {len(synced_indicators)} показателей",
        ...
        await session.commit()
        logger.exception("world_bank_sync_failed", error=exc.message)
        return {"ok": False, "error": exc.code, "message": exc.message}
    except Exception:
        await session.rollback()
        provider.last_sync_status = "error"
        await session.commit()
        logger.exception("world_bank_sync_failed")
