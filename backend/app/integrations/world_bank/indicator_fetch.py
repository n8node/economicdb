...
from app.integrations.world_bank.client import WorldBankError, fetch_indicator_series as wb_fetch
...
async def fetch_world_bank_indicator(
    ...
    *,
    ...
    if not external_id:
        raise WorldBankError(f"Нет external_id для {indicator.id}", code="world_bank_missing_external_id")
    try:
        indicator_code, country_code = _split_code_country(external_id)
    except ValueError as exc:
        raise WorldBankError(f"Неверный external_id: {external_id}", code="world_bank_bad_external_id") from exc
    series = await wb_fetch(country_code, indicator_code, from_date=from_date, to_date=to_date)
    if not series:
        raise WorldBankError(f"Пустой ряд World Bank {external_id}", code="world_bank_empty_series")
